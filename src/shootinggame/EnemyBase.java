...
public class EnemyBase extends Enemy {
	...
	public void move() {
		...
		if(Math.random()<0.01+GameWorld.stage*0.001) {
		...
		if (Math.random()<0.02+GameWorld.stage*0.001) {
			GameWorld.enemies.add(new DropEnemy(x,y,0,GameWorld.stage));
		}
		if (Math.random()<0.01+GameWorld.stage*0.001) {
			GameWorld.enemies.add(new CurveEnemy(x,y,0,GameWorld.stage));
		}
		if (Math.random()<0.05+GameWorld.stage*0.001) {
			GameWorld.enemies.add(new OrijinaruEnemy(x,y,0,GameWorld.stage));
		}
